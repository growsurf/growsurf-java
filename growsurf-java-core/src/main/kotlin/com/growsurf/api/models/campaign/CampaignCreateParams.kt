...
private constructor(
    ...
    /**
     * What the program is for, which seeds the share buttons and the starter rewards that suit that
     * audience. Programs whose participants refer other businesses (`CUSTOMERS`, `USERS`,
     * `B2B_SAAS_SELF_SERVICE`, `B2B_SAAS_ENTERPRISE`, `HEALTHCARE_PROVIDERS`) start with the
    ...
    class Builder internal constructor() {
        ...
        /**
         * What the program is for, which seeds the share buttons and the starter rewards that suit
         * that audience. Programs whose participants refer other businesses (`CUSTOMERS`, `USERS`,
         * `B2B_SAAS_SELF_SERVICE`, `B2B_SAAS_ENTERPRISE`, `HEALTHCARE_PROVIDERS`) start with the
         * LinkedIn share button visible; consumer, financial, education, insurance, telehealth,
         * newsletter, and waitlist programs (`B2C_SUBSCRIPTIONS`, `FINANCIAL_SERVICES`,
    ...
    private constructor(
        ...
        class Builder internal constructor() {
             ...
             * suit that audience. Programs whose participants refer other businesses (`CUSTOMERS`,
             * `USERS`, `B2B_SAAS_SELF_SERVICE`, `B2B_SAAS_ENTERPRISE`, `HEALTHCARE_PROVIDERS`)
    ...
    private constructor(
        ...
        /**
         * The event that earns this Campaign Reward. Applies to `SINGLE_SIDED`, `DOUBLE_SIDED`, and
         * `MILESTONE` rewards. When omitted, the Campaign Reward uses `CONVERSION`. `LEAD` requires
         * the program's `installation.referralTrigger` to be `CUSTOM`.
         *
         * @throws GrowsurfInvalidDataException if the JSON field has an unexpected type (e.g. if
         *   the server responded with an unexpected value).
         */
        ...
        class Builder internal constructor() {
            ...
            /**
             * The event that earns this Campaign Reward. Applies to `SINGLE_SIDED`, `DOUBLE_SIDED`,
             * and `MILESTONE` rewards. When omitted, the Campaign Reward uses `CONVERSION`. `LEAD`
             * requires the program's `installation.referralTrigger` to be `CUSTOM`.
             */
        ...
        override fun toString() =
            "Reward{type=$type, title=$title, description=$description, event=$event, referralDescription=$referralDescription, imageUrl=$imageUrl, isVisible=$isVisible, isUnlimited=$isUnlimited, referredRewardUpfront=$referredRewardUpfront, limit=$limit, conversionsRequired=$conversionsRequired, numberOfWinners=$numberOfWinners, order=$order, limitDuration=$limitDuration, nextMilestonePrefix=$nextMilestonePrefix, nextMilestoneSuffix=$nextMilestoneSuffix, couponCode=$couponCode, referralCouponCode=$referralCouponCode, metadata=$metadata, commissionStructure=$commissionStructure, value=$value, referredValue=$referredValue, additionalProperties=$additionalProperties}"
