 ...
 * when applicable.
 *
 * At least one transaction identifier is required: one of `externalId`, `transactionId`, `orderId`,
 * `paymentId`, `invoiceId`, `paymentIntentId`, or `chargeId`. `customerId` and `subscriptionId` do
 * not count, since they identify the customer or subscription rather than the specific transaction.
 * Without an identifier, resending the same sale creates a duplicate commission and double-pays the
 * referrer; the server rejects such requests with HTTP 400.
 */
