 ...
 * account starts a 14-day Business trial without a credit card. The response includes an API key for
 * the new account, shown once in the response. The key is a secret: store it in a secret manager and
 * do not put it in logs, screenshots, URLs, model context, analytics, or generated output. A lost key
 * cannot be recovered through this API, so do not create an account here unless you can store the key
 ...
 * days. Separately, for security, the API key is replaced the first time the account owner signs in
 * to the GrowSurf dashboard; email verification does not trigger that, and the previous key then
 * returns a `403` with error code `NOT_AUTHORIZED_ERROR`. Some actions (such as emailing
 * participants) additionally require GrowSurf to verify the team first. Calling this endpoint accepts
