...
sealed class JsonField<out T : Any> {
    ...
    fun asStringOrThrow(): String =
        asString().orElseThrow { GrowsurfInvalidDataException("Value is not a string") }
    ...
    internal fun getRequired(name: String): T =
        when (this) {
            is KnownValue -> value
            is JsonMissing -> throw GrowsurfInvalidDataException("`$name` is not set")
            is JsonNull -> throw GrowsurfInvalidDataException("`$name` is null")
            else -> throw GrowsurfInvalidDataException("`$name` is invalid, received $this")
        }
