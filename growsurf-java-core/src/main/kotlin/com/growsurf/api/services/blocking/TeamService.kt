...
interface TeamService {
     ...
     * This is the recovery path for a `403` with error code `EMAIL_NOT_VERIFIED_ERROR`, so it stays
     * callable with any GrowSurf API key while the rest of the API is locked. The response never
     * reveals the owner's email address. A `200` with `status: SENT` is returned only when an email
     * was actually dispatched. Returns `400` if the email is already verified, and `429` if a
